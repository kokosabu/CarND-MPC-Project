...
const double cte_weight       = 1000.0;
const double epsi_weight      = 1000.0;
const double v_weight         =   10.0;
const double delta_weight     =  300.0;
const double a_weight         =   20.0;
const double delta_gap_weight =   10.0;
const double a_gap_weight     =    5.0;
...
class FG_eval {
 public:
  ...
  // The x and y coordinates are contained in the ptsx and ptsy vectors.
  // Since these are 2-element vectors a 1-degree polynomial (straight line) is sufficient.
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for(int t = 0; t < N - 2; t++) {
      fg[0] += delta_gap_weight * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += a_gap_weight     * CppAD::pow(vars[a_start     + t + 1] - vars[a_start     + t], 2);
    }
    ...
    for(int t = 1; t < N; t++) {
      ...
      AD<double> psides0 = CppAD::atan(coeffs[1] + 2.0*coeffs[2]*CppAD::pow(x0,1) + 3.0*coeffs[3]*CppAD::pow(x0,2));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for(int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -rad;
    vars_upperbound[i] =  rad;
  }
